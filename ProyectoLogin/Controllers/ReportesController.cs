    ...
    public class ReportesController : Controller
    {
        ...
        public async Task<IActionResult> Ventas(DateTime? desde, DateTime? hasta, string vendedor, string formaPago)
        {
            ...
            var ventas = await query
                ...
                .Select(v => new
                {
                    Fecha = FechaLocal.ConvertirDeUtc(v.FechaVenta), // 🔹 CONVERTIR A HORA LOCAL
                        ...
                        content.Column(column =>
                        {
                            ...
                            column.Item().Table(table =>
                            {
                                ...
                                foreach (var v in ventas)
                                {
                                    ...
                                    var imp = v.Total - sub;
                                    var fechaLocal = FechaLocal.ConvertirDeUtc(v.FechaVenta); // 🔹 CONVERTIR A LOCAL PARA PDF
                                    ...
                                    table.Cell().BorderBottom(0.5f).Padding(1).Text(TruncateText($"{v.Cliente.Nombres} {v.Cliente.Apellidos}", 20)).FontSize(8);
                                    table.Cell().BorderBottom(0.5f).Padding(1).Text(TruncateText(v.Cliente.Nit, 12)).FontSize(8);
